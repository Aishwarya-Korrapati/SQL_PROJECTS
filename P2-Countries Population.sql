...
/*5.Find the Total Population of All Countries*/
SELECT SUM(population) AS Total_population FROM countries;

/*List All Countries by Population in Descending Order*/
SELECT name, population FROM countries ORDER BY population DESC;

/*Find Countries with a Population Change Greater than 1 Million*/
SELECT name, population_change FROM countries WHERE population_change > 1000000;

/*Get the Average Population Density*/
SELECT AVG(density_per_sq_km) AS average_density FROM countries;

/*Retrieve Countries with a Fertility Rate Above 2.5*/
SELECT name, fertility_rate FROM countries WHERE fertility_rate > 2.5;

/*Find Countries with a Median Age Less than 30*/
SELECT name, median_age FROM countries WHERE median_age < 30;

/*Show Countries with Positive Net Migration*/
SELECT name, net_migrants FROM countries WHERE net_migrants > 0;

/*List Countries by Density (People per Sq Km) in Descending Order*/
SELECT name, density_per_sq_km FROM countries ORDER BY density_per_sq_km DESC;

/*Find Countries Representing More than 1% of the World Population*/
SELECT name, percent_of_world_pop FROM countries WHERE percent_of_world_pop > 1;

/*Calculate the Average Fertility Rate for Countries with a Population Over 100 Million*/
SELECT AVG(fertility_rate) AS average_fertility_rate FROM countries WHERE population > 100000000;

/*Identify the Top 5 Most Populous Countries*/
SELECT name, population FROM countries ORDER BY population DESC LIMIT 5;

/* Get the Total Area of All Countries Combined*/
SELECT SUM(area_sq_km) AS total_area_sq_km FROM countries;

/*Find Countries with Negative Population Growth (Decrease in Population)*/
SELECT name, percent_one_year_change FROM countries WHERE percent_one_year_change < 0;

/*Get the Country with the Highest Density*/
SELECT name, density_per_sq_km FROM countries ORDER BY density_per_sq_km DESC LIMIT 1;

/*Show the Countries Ordered by Fertility Rate in Descending Order*/
SELECT name, fertility_rate FROM countries ORDER BY fertility_rate DESC;

/*Find the Country with the Highest Population Growth Rate*/
SELECT name, percent_one_year_change FROM countries ORDER BY percent_one_year_change DESC LIMIT 1;

/*Get Countries where Fertility Rate is Below the World Average*/
SELECT name, fertility_rate FROM countries WHERE fertility_rate < (SELECT AVG(fertility_rate) FROM countries);

/*Find the Average Fertility Rate for Countries Grouped by Median Age Range*/
SELECT 
    CASE 
...
/*Get the Country with the Largest Population Decrease (Negative Growth)*/
SELECT name, population_change FROM countries WHERE population_change < 0 ORDER BY population_change ASC LIMIT 1;

/*Find the Population Percentage Distribution for Each Country*/
SELECT name, (population * 100.0 / (SELECT SUM(population) FROM countries)) AS percent_distribution FROM countries;

/*Identify Countries that Have Both High Density and Low Fertility Rates*/
SELECT name, density_per_sq_km, fertility_rate FROM countries WHERE density_per_sq_km > 300 AND fertility_rate < 2.0;

/*Calculate Total Population and Average Median Age for Each Fertility Rate Range*/
SELECT 
    CASE 
        WHEN fertility_rate < 2 THEN 'Below 2'
        WHEN fertility_rate BETWEEN 2 AND 3 THEN '2-3'
        ELSE 'Above 3'
...
/*Identify Countries with Population Growth Above Average and Net Migration Below Average*/
SELECT name, percent_one_year_change, net_migrants FROM countries 
WHERE percent_one_year_change > (SELECT AVG(percent_one_year_change) FROM countries) 
AND net_migrants < (SELECT AVG(net_migrants) FROM countries);

/*Rank Countries by Population Density and Retrieve Rank for a Specific Country*/
            /*Using a Subquery*/
SELECT name, density_per_sq_km, density_rank
FROM (
    SELECT name, density_per_sq_km, 
           RANK() OVER (ORDER BY density_per_sq_km DESC) AS density_rank 
    FROM countries
) AS ranked_countries
WHERE name = 'India';
...
/* Find the Difference in Population Density Between Each Country and the Global Average*/
SELECT name, density_per_sq_km,
       density_per_sq_km - (SELECT AVG(density_per_sq_km) FROM countries) AS density_difference
FROM countries
ORDER BY density_difference DESC;

/*Calculate the Average and Total Population by Fertility Rate Range and Median Age Range*/
SELECT 
    CASE 
        WHEN fertility_rate < 2 THEN 'Low Fertility'
        WHEN fertility_rate BETWEEN 2 AND 3 THEN 'Medium Fertility'
        ELSE 'High Fertility'
    END AS fertility_range,
    CASE 
        WHEN median_age < 30 THEN 'Young'
        WHEN median_age BETWEEN 30 AND 45 THEN 'Middle-aged'
        ELSE 'Older'
    END AS age_range,
    AVG(population) AS average_population,
    SUM(population) AS total_population
FROM countries
GROUP BY fertility_range, age_range;

/*Calculate the Average Population Change for Each Net Migration Category (Positive, Neutral, Negative)*/
SELECT 
    CASE 
        WHEN net_migrants > 0 THEN 'Positive Migration'
        WHEN net_migrants = 0 THEN 'Neutral Migration'
        ELSE 'Negative Migration'
